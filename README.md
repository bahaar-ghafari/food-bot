...
- **`bot`** — built from the repo's multi-stage `Dockerfile` (Maven build → `eclipse-temurin:17-jre-alpine` runtime). Reads secrets from a sibling `.env` (`TELEGRAM_BOT_TOKEN`, `TELEGRAM_BOT_USERNAME`, `SUPERADMIN_CHAT_ID`, `FEEDBACK_CHAT_ID`, optionally `ANTHROPIC_API_KEY` — never committed). `foods.json`/`languages.json` persist under a bind-mounted `./data` directory, so they can be seeded or backed up directly from the host.
...
- `anthropic.api.key` / `ANTHROPIC_API_KEY` — optional. If set, powers the `/cook` "nothing matches" fallback with a Claude-generated suggestion. Not required: with no key, that fallback automatically uses a free lookup against [TheMealDB](https://www.themealdb.com/api.php)'s public test API (no signup) instead, so the feature works out of the box either way.
...
- `/addfood` (or the "➕ Add food" button) — guided flow: pick your personal list or the global list → name → prep time in minutes → category (tap one) → ingredients → optional amounts for each ingredient (see below) → an optional recipe, managed with the recipe-step manager described below. Once you tap "✅ Done" on the recipe, you land on a **review screen** showing everything you entered, with a button per field ("✏️ Name", "✏️ Time", "✏️ Category", "✏️ Ingredients", "✏️ Recipe") to jump back and fix anything before it's saved — tapping one revisits that step and returns you straight to the review when you're done, rather than replaying the whole wizard. Tap "✅ Confirm & Save" once it looks right. The whole flow (including the review screen) is a single message that morphs as you go, and every step has a "🔙 Back" button (Back on the very first question cancels the flow).
- `/menu` (or the "📋 All foods" button) — **superadmin only.** Pick your list, the global list, or "🌐 All" (both combined), then shows food names as tappable buttons, 10 per page with "◀️"/"▶️" navigation if there are more. Tap a food to see "🍽️" its name, "🏷️" category, "⏱️" prep time, "🧾" ingredients, and "📝" recipe, plus a "⚙️ Settings" button (→ "✏️ Edit" / "🗑️ Delete") on every food, personal or global. A "🔙 Back" button is always available to retrace your steps (list → detail → settings → edit-field choice) by editing the same message in place, rather than piling up new ones. A "🔄 Restart & refresh" button also sits at the bottom of each list page — the bot only reads `foods.json` into memory once at startup, so this re-reads it from disk in place (no process restart) to pick up any changes made outside the bot (e.g. a manually edited or restored data file) before re-rendering the same list. Non-admins don't see this button — they manage their own personal-list foods via `/search` instead (see below).
- `/cook` (or the "🍳 What can I cook?" button) — asks how much time you have (tap "⚡ Fast (~30m)", "🕐 ~2 hours", "🕔 ~5 hours", "📅 ~1 day", "♾️ Any", or type an exact number of minutes), then which ingredients you currently have (water, salt, oil, and spices aren't asked about — everyone's assumed to have those already, and they're optional even if not), whether you can go shopping for anything missing, and a category filter (or Any). Replies with two tappable, paginated lists: "✅ ready now" and "🍽️ could cook if you shop". Both are ranked to put the best use of what you already have first: the "could cook if you shop" list only includes dishes that actually reuse at least one of the ingredients you said you have (so telling it "I have chicken" won't surface an unrelated fish or beef dish that would need everything bought from scratch — that's a different recipe, not "cook what I have and buy the shortfall"), sorted by the *fewest* extra ingredients needed (minimum shopping); within dishes needing the same number of extras — or among "ready now" dishes, which all need zero — ties go to whichever dish reuses more of what you have (so if you've got chicken and several chicken dishes fit, an unrelated dish like Ghormeh Sabzi that just happens to need the same number of extras won't jump ahead of them). Tap any food to see a clean breakdown — what you already have, what you need to buy, and its recipe, with a "🔙 Back" button to return to that same result list. Considers both your list and the global list.
  - **When nothing in the list fits:** if you can shop, the bot tries Claude first (only if `ANTHROPIC_API_KEY` is set), then automatically falls back to a free, keyless recipe lookup on TheMealDB built around one of your ingredients, and only falls back further to a flat "nothing matches" if both of those come up empty. If shopping's off-limits, it first tries to point at a single protein or carb you already have (e.g. "you've got chicken — just cook that plain") rather than suggesting you eat everything raw; only if you have neither does it try the Claude/TheMealDB fallback chain, and only if that also comes up empty does it fall back to the (gentle) roast about eating your ingredients as-is or eating air if you picked nothing at all. TheMealDB results come back in English regardless of your language (the bot says so in Persian mode).
- `/search <text>` (or the "🔍 Search recipe" button, which prompts for the text instead) — searches your own list and the global list, in your current language, in order. The prompt also has three buttons so you can skip typing and browse instead — this is the one non-admin path to these views (`/menu` is superadmin-only): "🌍 Global list" (everyone's shared foods), "👤 My list" (foods you own — currently on your personal list), and "🍽️ My recipes" (everything *you* added, personal or global — so a recipe you contributed to the global list still shows up here even though it's no longer "yours" to edit). Search order:
...
Right after you finish picking ingredients (in `/addfood` or via "✏️ Edit" → ingredients), you land on an amounts step: each selected ingredient is shown as its own button, optionally suffixed with its current amount (e.g. "🥚 egg — 2"). Tap one to type an amount (max 30 characters) or type "skip"/"-" to leave it unset; tap "✅ Done" at any point to move on — the whole step is skippable, with no amounts entered by default. Amounts show up in parentheses next to the ingredient everywhere it's displayed (detail view, `/cook` breakdowns, the add-food review screen) and are dropped automatically if that ingredient is later removed.

### Editing

From a food's detail view → "⚙️ Settings" → "✏️ Edit", you (the food's owner, or the superadmin for any food) can change name, prep time, category, ingredients (which flows into the amounts step above before saving), or recipe independently.
...
Saved foods (name, category, prep time, ingredients with their optional amounts, and an optional recipe) are persisted to `foods.json`, and each chat's language choice to `languages.json`, both in the working directory. The ingredient buttons shown are built from ingredients already used in foods visible to you, so the list grows as you add more.
...
Unit tests cover `FoodRepository` (CRUD, ownership scoping, persistence, legacy-data ID backfill, reload-from-disk), `Food` (ingredient-amounts null-safety, `toString`), `FoodCategories`, `IngredientIcons`, `IngredientCategories` (protein/carb classification used by the `/cook` no-shop fallback), `IngredientTranslations`, `FoodNameTranslations`, `IngredientSearch` (the filter/cap logic behind the ingredient picker), `Paginator`, `PantryStaples`, `RecipeSteps`, `FoodSearch` (name/ingredient/fuzzy matching), `Levenshtein`, `TimeFormat`, `Messages` (catalog completeness), `LanguageRepository`, `CookResultContext` and `SearchResultContext` (defensive copying/insertion order), `TelegramConfig` (the `.env` → `application.properties` → environment-variable precedence rule the config getters are built on), `AiSuggestionService` (prompt building and the enabled/disabled gate, without hitting the network), and `MealDbSuggestionService` (ingredient-picking and recipe-formatting logic, without hitting the network). The Telegram-facing `FoodBot` class, `FoodBotApplication`, and `HealthCheckServer` aren't unit-tested — they're tightly coupled to the Telegram API's `execute()` calls, process wiring, and a real socket/environment variable respectively — so changes there should still be manually verified by running the bot.
