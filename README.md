...
- `superadmin.chat.id` / `SUPERADMIN_CHAT_ID` — the chat ID that can edit/delete *any* food (personal or global). Regular users can still edit/delete the foods on their own personal list. Find your numeric Telegram chat ID via a service like `@userinfobot`.
...
- `/search <text>` (or the "🔍 Search recipe" button, which prompts for the text instead) — searches by food name first (e.g. "ghorme" matches "Ghormeh Sabzi"), and only if nothing matches by name does it fall back to searching ingredients. A single match shows the recipe directly; multiple matches show tappable buttons (10 per page) that lead to the same detail view as `/menu`, with the same "🔙 Back"/"⚙️ Settings" behavior. No matches at all gets a clear "nothing found" reply. Searches only your own list and the global list, in your current language.
