...
Unit tests cover `FoodRepository` (CRUD, ownership scoping, persistence, legacy-data ID backfill, reload-from-disk), `FoodCategories`, `IngredientIcons`, `IngredientCategories` (protein/carb classification used by the `/cook` no-shop fallback), `IngredientTranslations`, `FoodNameTranslations`, `IngredientSearch` (the filter/cap logic behind the ingredient picker), `Paginator`, `PantryStaples`, `RecipeSteps`, `FoodSearch` (name/ingredient/fuzzy matching), `Levenshtein`, `TimeFormat`, `Messages` (catalog completeness), `LanguageRepository`, `AiSuggestionService` (prompt building and the enabled/disabled gate, without hitting the network), and `MealDbSuggestionService` (ingredient-picking and recipe-formatting logic, without hitting the network). The Telegram-facing `FoodBot` class itself isn't unit-tested — it's tightly coupled to the Telegram API's `execute()` calls — so changes there should still be manually verified by running the bot.
